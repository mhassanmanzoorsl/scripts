...
RESOURCE_GROUPS=("POD-COMMON-RG" "PL-DevOps-POC-RG")

# Set the output file for combined results
OUTPUT_FILE="azure_resources_tls_versions_summary.csv"

# Add headers to the CSV file
echo "Resource Group,Resource Type,Resource Name,TLS Version" > $OUTPUT_FILE
...
for RESOURCE_GROUP in "${RESOURCE_GROUPS[@]}"; do
  ...
  if [ -z "$APIM_SERVICES" ]; then
    ...
    for APIM_NAME in $APIM_SERVICES; do
      ...
      echo "$RESOURCE_GROUP,API Management,$APIM_NAME,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$FUNCTION_APPS" ]; then
    ...
    for FUNCTION_APP in $FUNCTION_APPS; do
      ...
      TLS_VERSION=${TLS_VERSION:-"TLS 1.2 (Default)"}
      echo "$RESOURCE_GROUP,Function App,$FUNCTION_APP,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$WEB_APPS" ]; then
    ...
    for WEB_APP in $WEB_APPS; do
      ...
      TLS_VERSION=${TLS_VERSION:-"TLS 1.2 (Default)"}
      echo "$RESOURCE_GROUP,Web App,$WEB_APP,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$APP_GATEWAYS" ]; then
    ...
    for APP_GATEWAY in $APP_GATEWAYS; do
      ...
      TLS_VERSION=${TLS_VERSION:-"TLS 1.2 (Default)"}
      echo "$RESOURCE_GROUP,Application Gateway,$APP_GATEWAY,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$COSMOS_DB_ACCOUNTS" ]; then
    ...
    for COSMOS_DB_ACCOUNT in $COSMOS_DB_ACCOUNTS; do
      ...
      echo "$RESOURCE_GROUP,Cosmos DB,$COSMOS_DB_ACCOUNT,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$MYSQL_SERVERS" ]; then
    ...
    for MYSQL_SERVER in $MYSQL_SERVERS; do
      ...
      TLS_VERSION=${TLS_VERSION:-"TLS 1.2 (Default)"}
      echo "$RESOURCE_GROUP,MySQL Server,$MYSQL_SERVER,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$SQL_SERVERS" ]; then
    ...
    for SQL_SERVER in $SQL_SERVERS; do
      ...
      TLS_VERSION=${TLS_VERSION:-"TLS 1.2 (Default)"}
      echo "$RESOURCE_GROUP,SQL Server,$SQL_SERVER,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$SYNAPSE_WORKSPACES" ]; then
    ...
    for SYNAPSE_WORKSPACE in $SYNAPSE_WORKSPACES; do
      ...
      TLS_VERSION=${TLS_VERSION:-"TLS 1.2 (Default)"}
      echo "$RESOURCE_GROUP,Synapse Workspace,$SYNAPSE_WORKSPACE,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$KEY_VAULTS" ]; then
    ...
    for KEY_VAULT in $KEY_VAULTS; do
      ...
      TLS_VERSION=${TLS_VERSION:-"TLS 1.2 (Default)"}
      echo "$RESOURCE_GROUP,Key Vault,$KEY_VAULT,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$LOGIC_APPS" ]; then
    ...
    for LOGIC_APP in $LOGIC_APPS; do
      ...
      echo "$RESOURCE_GROUP,Logic App,$LOGIC_APP,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ ${#WORKSPACES[@]} -eq 0 ]; then
    ...
    for WORKSPACE in "${WORKSPACES[@]}"; do
      ...
      echo "$RESOURCE_GROUP,Power BI Workspace,$WORKSPACE,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi
  ...
  if [ -z "$SERVICE_BUS_NAMESPACES" ]; then
    ...
    for NAMESPACE in $SERVICE_BUS_NAMESPACES; do
      TLS_VERSION="TLS 1.2 (Default)"  # Assuming default TLS version
      echo "$RESOURCE_GROUP,Service Bus,$NAMESPACE,$TLS_VERSION" >> $OUTPUT_FILE
    done
  fi

done

echo "Summary of TLS versions for all resources has been written to $OUTPUT_FILE"
